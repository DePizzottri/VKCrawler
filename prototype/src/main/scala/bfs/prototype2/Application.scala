...
object Application extends App {
  ...
  val settings = args.toSet

  val system = ActorSystem("bfs-system", config)

  if(!settings.contains("deploy")) {
    ...
    println(s"""Actor system deployed on ${config.getString("akka.remote.netty.tcp.hostname")}:${config.getString("akka.remote.netty.tcp.port")}""")
  }
}